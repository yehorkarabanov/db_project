...
const ProductEdit = () => {
    ...
    const fetchProductData = async () => {
        try {
            ...
            setFormData({
                ...
                place_taken: product_details.product.place_taken,
                warehouse_id: product_details.warehouse.id,
                manufacturer_id: product_details.manufacturer.id,
                types: product_details.types.map((type) => type.type_id.toString()),
            });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const encodedProductName = encodeURIComponent(product_name);
            await axios.put(
                `http://localhost:8080/api/products/${encodedProductName}/`,
                formData
            );
            navigate(`/products/${encodeURIComponent(formData.name)}`);
        } catch (err) {
            console.error(err);
